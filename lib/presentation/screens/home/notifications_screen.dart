...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _loadNotifications() async {
    ...
    final userId = currentContext.read<UserCubit>().state.user?.id;
    ...
    if (userId != null) {
      ...
      await notificationCubit.getNotifications(userId);

      // Check if widget is still mounted before accessing state
      if (mounted) {
        // Log the loaded notifications
        final notifications = notificationCubit.state.notifications;
        debugPrint('Loaded ${notifications.length} notifications');
        for (var notification in notifications) {
          debugPrint(
            '- ${notification.id}: ${notification.title} (${notification.isRead ? 'read' : 'unread'})',
          );
        }
      }
    } else {
      debugPrint('Cannot load notifications: User ID is null');
    }
  }
  ...
  Future<void> _markAllAsRead() async {
    ...
    if (userId != null) {
      // Get the notification cubit before async operation
      final notificationCubit = currentContext.read<NotificationCubit>();
      await notificationCubit.markAllNotificationsAsRead(userId);
  ...
  Future<void> _deleteNotification(String notificationId) async {
    ...
    // Get the notification cubit before async operation
    final notificationCubit = currentContext.read<NotificationCubit>();
    await notificationCubit.deleteNotification(notificationId);
          ...
          onTap: () async {
            ...
            if (!notification.isRead) {
              ...
              final notificationCubit =
                  currentContext.read<NotificationCubit>();
              await notificationCubit.markNotificationAsRead(notification.id);
            }
            ...
            if (notification.relatedRoomId != null && mounted) {
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder:
                      (context) =>
                          RoomDetailScreen(roomId: notification.relatedRoomId!),
                ),
              );
            }
          },
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: iconColor.withAlpha(25), // 0.1 opacity (25/255)
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      // Date
                      Text(formattedDate, style: AppTheme.captionStyle),
                    ],
                  ),
                ),
                // Read indicator
