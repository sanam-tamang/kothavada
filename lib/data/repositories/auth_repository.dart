...
class AuthRepository {
  ...
  Future<UserModel> signIn({
    ...
    try {
      // First, ensure any existing session is cleared to prevent using previous credentials
      ...
      if (response.user == null) {
        _logger.e('Authentication failed: No user returned from Supabase');
        throw Exception(AppConstants.authErrorMessage);
      }
      ...
      _logger.e('Authentication error: ${e.message}');
      // Make sure to sign out to clear any existing session
      try {
        await _supabaseClient.auth.signOut();
      } catch (signOutError) {
        _logger.e('Error signing out after auth failure: $signOutError');
      }
      throw Exception(e.message);
    } catch (e) {
      ...
      try {
        ...
        _logger.e('Error signing out after general failure: $signOutError');
      }
      throw Exception(e.toString());
    }
  }
